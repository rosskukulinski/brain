...
tags: [tailscale, networking, distributed-systems, infrastructure, vision, peer-to-peer]
status: complete
project: TailScale
---
...
Avery Pennarun presents Tailscale's foundational vision: building a "New Internet" that reverses decades of unnecessary centralization. The core thesis is that modern software complexity doesn't stem from computational challenges but from connectivity barriers created by IPv4 limitations, firewalls, NATs, and the resulting client-server architecture. These constraints forced developers toward cloud providers like AWS, who charge "rent" for what should be basic connectivity.

Pennarun argues that the tech industry swings like a pendulum between centralized (IBM mainframes) and distributed (Microsoft PCs) computing, and we're currently in another centralized era despite having supercomputers in our pockets that function as "dumb terminals." The article critiques over-engineering for scale ("0.2 requests per second" needing Kubernetes) and the resulting layers of complexity: container orchestration, logging delays, deployment overhead.

Tailscale's solution: give every device certificates, static IP addresses, DNS names, and peer status—capabilities previously exclusive to servers—enabling direct peer-to-peer communication. Taildrop exemplifies this: file transfer becomes a simple HTTP PUT request instead of cloud uploads/downloads with their associated costs, encryption key management, and complexity. The vision requires near-universal adoption (currently 1 in 20,000 people globally), which explains their dual strategy of free personal plans and enterprise focus. Success would enable entirely new application architectures impossible in centralized systems, making the "easy things easy" again.
...
*Originally published at [tailscale.com/blog/new-internet](https://tailscale.com/blog/new-internet)*
